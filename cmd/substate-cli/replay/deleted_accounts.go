...
var GetDeletedAccountsCommand = cli.Command{
	Action:    getDeletedAccountsAction,
...
func readAccounts(ch chan state.AccountStatus) ([]common.Address, []common.Address) {
...
// getDeletedAccountsTask replays a transaction substate.
func getDeletedAccountsTask(config ReplayConfig, block uint64, tx int, recording *substate.Substate, ddb *substate.DestroyedAccountDB) error {
	...
	//wrapper
	ch := make(chan state.AccountStatus, 10000)
	statedb = state.NewProxyRecorder(statedb, ch)
...
func getDeletedAccountsAction(ctx *cli.Context) error {
	var err error
	...
	ddb := substate.OpenDestroyedAccountDB("./deletedDB-mainnet")
	defer ddb.Close()
	...
	for iter.Next() {
		...
		err := getDeletedAccountsTask(config, tx.Block, tx.Transaction, tx.Substate, ddb)
		if err != nil {
			return err
		}
		txCount++
